...
// Returns the cropped image as positioned on the overlay
- (UIImage *)croppedImage {
    ...
    NSLog(@"'visibleRect' size: width %f by height %f", visibleRect.size.width, visibleRect.size.height);
    
    // Transform visibleRect according to image orientation
    CGAffineTransform rectTransform = [self _orientationTransformedRectOfImage:self.imageToCrop];
    visibleRect = CGRectApplyAffineTransform(visibleRect, rectTransform);
    NSLog(@"'visibleRect' size after transform: width %f by height %f", visibleRect.size.width, visibleRect.size.height);
    ...
    NSLog(@"'result' size: width %f by height %f", result.size.width, result.size.height);
