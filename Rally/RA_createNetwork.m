...
- (void)viewDidLoad
{
   ...
   // Adjust the colours of the cells and labels
    ...
    self.largeStockSquash = [squash resizedImage:PF_USER_PIC_MENU_SIZE interpolationQuality:kCGInterpolationDefault];
    self.mediumStockSquash = [self.largeStockSquash getImageResizedAndCropped:PF_USER_PIC_MEDIUM_SIZE];
    self.largeStockTennis = [tennis resizedImage:PF_USER_PIC_MENU_SIZE interpolationQuality:kCGInterpolationDefault];
    ...
    self.ladderBlurb = @"The competition will be run according to ladder rules";
    self.leagueBlurb = @"The competition will be run according to league rules";
...
-(void) resetToDefault
{
    ...
    // Set default image to appear on league photo
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if(textField.tag == 1) {
        [self.userNameActivity startAnimating];
        if([self checkUsername:self.userName.text]){
            [self.userNameActivity stopAnimating];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([[tableView cellForRowAtIndexPath:indexPath].reuseIdentifier isEqualToString:@"league_photo"]) {
        ...
        self.imagePicker.cropSize = CGSizeMake(PF_USER_PIC_LARGE_WIDTH * 0.4, PF_USER_PIC_MENU_HEIGHT * 0.4);
...
- (void)imagePicker:(GKImagePicker *)imagePicker pickedImage:(UIImage *)image
{
    ...
    UIImage *picLarge = [image resizedImage:PF_USER_PIC_MENU_SIZE interpolationQuality:kCGInterpolationDefault];
