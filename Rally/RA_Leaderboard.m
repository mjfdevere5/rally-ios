...
{
    // Dequeue
    RA_LeaderboardCell *cell = [tableView dequeueReusableCellWithIdentifier:@"table_ladder_cell" forIndexPath:indexPath];
    
    // Alternating colour
    if ([indexPath row] % 2) { [cell setBackgroundColor:[UIColor whiteColor]]; }
    else { [cell setBackgroundColor:UIColorFromRGB(0xE0EEEE)]; }
    
    // Rank
    NSString *rankNumber = [NSString stringWithFormat:@"%i",(int)(indexPath.row + 1)];
    cell.rankNumber.text = rankNumber;
    ...
    cell.ladderScore.text = [NSString stringWithFormat:@"%@",[self.scoresToIds objectForKey:user.objectId]];
